      ...
      try {
        ...
        const mapped = (response.data?.data || []).map(c => ({
          ...
          timeline: [
            ...
            { label: 'AI Classified', done: c.aiClassified },
            { label: `Assigned to ${c.department || 'Department'}`, done: !!c.updates?.[0] },
            { label: 'Under Review', done: c.status === 'in_progress' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 900 }} className="animate-fade-up">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {[
          { label: 'Total Filed',   value: complaints.length, icon: FileText,       color: 'var(--blue)'    },
